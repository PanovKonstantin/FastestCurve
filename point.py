...
class Point:
    ...
    def init_surface(self):
        color = conf.RED if self.selected else (100, 100, 100)
        self.surface = pygame.Surface(
            (conf.POINT_R*2, conf.POINT_R*2), pygame.SRCALPHA, 32)
        self.surface = self.surface.convert_alpha()
        pygame.draw.circle(self.surface, color,
                           (conf.POINT_R, conf.POINT_R), conf.POINT_R)
        self.set_rectangle()
    ...
    def init_plots(self):
        pos = np.array(conf.PLOT_POSITION)
        inc = np.array([0, conf.PLOT_PADDING + conf.PLOT_HEIGHT])
        self.acceleration_plot = PlotGraph([self.t, 0], pos, "Acceleration")
        self.velocity_plot = PlotGraph([self.t, 0], pos + inc, "Velocity")
        self.kinetic_energy_plot = PlotGraph(
            [self.t, self.get_kintetic()], pos + inc*2, "Kinetic energy")
        self.potential_energy_plot = PlotGraph(
            [self.t, self.get_potential()], pos + inc*3, "Potential energy")
        self.total_energy_plot = PlotGraph(
            [self.t, self.get_total()], pos + inc*4, "Total energy")
        self.total_energy_loss_plot = PlotGraph(
            [self.t, self.get_total_energy_loss()], pos + inc*5, "Total energy loss")
    ...
    def find_next_position(self):
        ...
        while self.get_pos(step) * step <= self.position * step:
            self.index += step
            if (self.check_overflow_max()):
                break
            if (self.check_overflow_zero()):
                break
    ...
    def update_acceleration(self):
        self.acceleration = -conf.parameters['G'] * self.k
    ...
    def update_friction(self):
        ...
        Fn = conf.parameters['M'] * conf.parameters['G'] * k_sin
        self.friction = conf.parameters['MU'] * Fn
    ...
    def set_rectangle(self):
        ...
        self.rectangle = self.rectangle.move(-conf.POINT_R, -conf.POINT_R)
        self.rectangle = self.rectangle.move(conf.GRAPH_POSITION)
        xy = self.get_xy() * conf.GRAPH_STEP / conf.parameters['SCALE']
        xy = conf.coord_to_screen(xy, self.table['min_x'], self.table['max_y'])
    ...
    def plot(self):
        ...
        self.total_energy_loss_plot.plot(
            [self.t, self.get_total_energy_loss()])
    ...
    def get_kintetic(self):
        return self.velocity**2 * conf.parameters['M'] / 2

    def get_potential(self):
        return conf.parameters['M'] * conf.parameters['G'] * (self.get_xy()[1] - self.table['min_y'])
...
class PlotGraph:
    ...
    def reset_surface(self):
        self.surface = pygame.Surface(conf.PLOT_SIZE, pygame.SRCALPHA, 32)
    ...
    def rescale(self):
        while abs(self.values[-1][1]) >= self.scale/2:
            self.scale *= 5

    def transform(self, value):
        new_value = [0, 0]
        new_value[0] = (value[0] - self.initial_time) * self.time_scale
        new_value[1] = (-value[1] / self.scale) * \
            conf.PLOT_HEIGHT + conf.PLOT_HEIGHT/2
        return new_value
    ...
    def blit(self):
        ...
        constr_width = int(self.surface.get_width()) - \
            (int(self.surface.get_width()) % step)
        constr_height = int(self.surface.get_height()) - \
            (int(self.surface.get_height()) % step)
        current_value = self.values[-1][1]
        if abs(current_value) < 0:
            current_value = round(current_value, 6)
        else:
            current_value = round(current_value, 2)
        name = conf.my_font.render(
            self.name + f':   {current_value}', True, conf.BLACK)
        conf.screen.blit(name, self.rectangle.move(0, -30))
        for i in range(0, constr_width + 1, step):
            pygame.draw.line(conf.screen, color, (x + i, y),
                             (x + i, y + constr_height))
        for i in range(0, constr_height + 1, step):
            ...
            text = conf.my_font10.render(f"{y_axis}", True, conf.BLACK)
            conf.screen.blit(text, (x - 10 - text.get_width(), y + i - 10))
            pygame.draw.line(conf.screen, color, (x, y + i),
                             (x + constr_width, y + i))
        conf.screen.blit(self.surface, self.rectangle)
