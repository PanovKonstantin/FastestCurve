...
class DraggablePoint:
    def __init__(self, xy, min_x, max_y, position=GRAPH_POSITION) -> None:
        self.xy = xy
        self.min_x = min_x
        self.max_y = max_y
        self.is_dragging = False
        self.position = position
        self.screen_xy = coord_to_screen(self.xy, self.min_x, self.max_y)
        self.init_surface()
        self.update_screen()
    ...
    def is_stop_drag_event(self, event):
        if event.type != pygame.MOUSEBUTTONUP: return False
        if event.button != 1: return False
    ...
    def is_drag_event(self, event):
        if event.type != pygame.MOUSEMOTION: return False
        if not self.is_dragging: return False
        xy = event.pos
        if xy[0] < self.minx: return False
        if xy[0] > self.maxx: return False
        if xy[1] < self.miny: return False
        if xy[1] > self.maxy: return False
        return True
    ...
    def handle_drag(self, event, pause, resume, redraw):
        if self.is_start_drag_event(event):
            ...
            self.xy = screen_to_coord(self.screen_xy, self.min_x, self.max_y)
            redraw(True)
            self.update_screen()
