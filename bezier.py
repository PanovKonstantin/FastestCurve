...
class Bezier:
    def __init__(self, ctrl) -> None:
        ...
        self.gen_table()
        self.init_surface()
        self.ctrl_drag = [DraggablePoint(xy * GRAPH_STEP / parameters['SCALE'], self.table['min_x'], self.table['max_y']) for xy in self.ctrl]
        for c in self.ctrl_drag:
            c.set_constraints(GRAPH_POS_X,
            GRAPH_POS_X + GRAPH_WIDTH_CONSTR - GRAPH_STEP*2,
            GRAPH_POS_Y,
            GRAPH_POS_Y + GRAPH_HEIGHT_CONSTR - GRAPH_STEP*2)
    ...
    def redraw(self, quick=False):
        self.ctrl = [p.xy * parameters['SCALE'] / GRAPH_STEP for p in self.ctrl_drag]
        if quick:
            self.gen_ts(100)
            self.gen_xy()
        else:
    ...
    def measure_szie(self):
        ...
        self.table['min_x'] = 0
        self.table['max_y'] = GRAPH_HEIGHT/2
    ...
    def init_surface(self):
        ...
        if (len(self.table['xy']) >= 2):
            ...
            xys = self.table['xy']
            line = [coord_to_screen(xy * GRAPH_STEP / parameters['SCALE'], minx, maxy) for xy in xys]
    ...
    def blit(self):
        screen.blit(self.surface, GRAPH_POSITION)
        if self.selected:
            for c in self.ctrl_drag:
                c.blit()
        else:
